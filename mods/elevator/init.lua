...
minetest.register_globalstep(function(dtime)
    ...
    for name,r in pairs(riding) do
        ...
        local ok = r.box and r.box.getpos and r.box:get_pos() and r.box:get_luaentity() and r.box:get_luaentity().attached == name
        if not ok then
...
for _,mode in ipairs({"on", "off"}) do
    ...
    minetest.register_node(nodename, {
        ...
        on_place = function(itemstack, placer, pointed_thing)
            ...
            if node == nil or (minetest.registered_nodes[node.name] and minetest.registered_nodes[node.name].drawtype ~= "airlike" and node.name ~= placeholder) then
                return
            end
