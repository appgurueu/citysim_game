...
- [x] **[SILENT]** ⚠ **Bone overrides on bones with a 180° bind rotation changed meaning in
      5.11.** ([5.11](#511)) Until then Irrlicht's `getScale()` decomposed a perfect
      half-turn bind rotation into a *negative scale* instead of a rotation (its
      "no off-diagonal entries ⇒ no rotation" shortcut is simply false), and the engine
      then applied that leftover on top of every override. An override that set rotation
      `R` therefore rendered as `R · R_bind`. 5.11 fixed the decomposition, so `R` now
      renders as `R`, and every mod that had tuned its numbers against the bug is off by
      a half-turn. Background: <https://luatic.dev/posts/breaking-bones/>.
      — audited 2026-07-30. **Both bone-override sites in the tree are affected.**
      Every player model (`player_api`, `default`, `medical` ×4, `3d_armor`) has a
      180° bind rotation on `Body` (about Y) and on `Arm_*`/`Leg_*`/`Cape` (about X);
      only `Head` is clean, which is why `headanim`-style mods escaped. Every `cars`
      model with a `steering` bone has one too (about Y). Both fixed — see §7.4.
      **Both fixes need a 5.9+ client**, and `cars` additionally regresses 5.9/5.10
      clients (see its row). Neither is verifiable headlessly.
      Note the three possible workarounds are **not** equivalent here: forcing the
      *correct* decomposition (what we do) needs a 5.9+ client, forcing the *wrong* one
      needs no rotation changes but re-introduces the bug, and wiggling the model's
      rotations is the only one that also covers pre-5.9 clients.
...
| 2026-07-30 | §1.3 bone semantics, §7.4 | **The 5.11 bone-override change.** `modlib` and `cmdlib` restored as submodules (rolling-94 → rolling-104); both were proved byte-identical to upstream first, so §7.2's band-2 rating for them was an artefact of counting pointer bumps — record the negative results. `character_anim` then fixed for the new semantics *and* the new modlib b3d behaviour without retuning a single hand-tuned pose, by converting bind-relative → absolute at one boundary. `cars` fixed too, rotation-only and knowingly trading 5.9/5.10 clients for 5.11+ ones. luacheck 3338 → **3280** warnings, still 0 errors — the whole delta is the newer `modlib`/`cmdlib` sources being linted instead of the old ones; the two hand-edited files are unchanged at 24 (`character_anim`) and 144 (`cars/init.lua`) warnings, all pre-existing. Nothing here is verifiable headlessly — it needs a client. |
...
| `modlib` | **at upstream HEAD** | **yes — exact tree match**, negative result | — | Was a submodule until `d5f444ef` de-submoduled it; **restored as a submodule 2026-07-30**. The vendored copy was byte-identical to upstream `7989f406` (tree `ff8db063`, found by scanning every upstream commit tree), so nothing was lost — despite §7.2's band-2 "small divergence", which only counted the pointer bumps. Note the gitlink recorded at `d5f444ef^` (`934ab6b1`) is *not* what was on disk; it predates `b3d.lua` entirely. rolling-94 → rolling-104. |
| `cmdlib` | **at upstream HEAD** | **yes — exact tree match**, negative result | — | Same story, same day: vendored tree `8b64fc05` == upstream `27095715`, restored as a submodule. |
| `character_anim` | fixed, **needs visual check** | no | §1.2 bone API, §1.3 bone semantics | Degrees→radians conversion happens at one boundary; the mod is degrees throughout. Its monkeypatch of `PlayerRef.set_bone_position` is **kept on purpose** — it exists to track third-party callers of the deprecated method — but forwards to `set_bone_override`. **The 5.11 bone-override change (§1.3) is now handled**, and it interacts with the modlib bump: rolling-103 dropped `b3d:get_animated_bone_properties`' precomposition with the bind rotation, which had been the mod's half of the same bug. Rather than retune the fork's hardcoded poses (cuffs, hands-up, `spriteguns`, `swordfighting`) and the euler tweaks that read them, **the whole pose pipeline was left in its bind-relative convention** and converts to absolute once, in `apply_bone`: the bind rotation is composed back in there, and the bind scale is pinned to stop a 5.9/5.10 client applying it twice. Verified numerically against the real `character.b3d` (192 bone-frames, rendered rotation identical to what a pre-5.11 client showed, worst delta 0). Needs `modlib.version >= 103`. Watch out: `quaternion.from_euler_rotation` is **not** the inverse of `to_euler_rotation` (Ry·Rx·Rz vs Rz·Ry·Rx), so the mod carries its own inverse. Upstream is LMD/character_anim, which solved this differently (model-side rotation wiggling); the fork's hardcoded animations are what make that route expensive here. |
| `cars` | partly migrated, bone rotation fixed **with a known regression** | no | §1.1, §1.2, §1.3 | Steering bone → `set_bone_override`; `weight` removed (6 sites incl. `newcars.lua`, `car01.lua`); `get_node_group`, velocity and LMB accessors. Note the `set_attach` call next to the bone override still takes **degrees**. §1.3: every model with a `steering` bone has a 180°-about-Y bind rotation, uniform across all 15, so the fix is a closed form — legacy `(0, 0, z)` becomes `(180, 0, 180 + z)` degrees (verified to 5e-16 over the whole `wheelpos` range). **Deliberately rotation-only**, unlike `character_anim`: pinning the scale as well would need the per-model bind scale, because the `jackhammer*` models have **6.5**, not 1 — that means reading the b3d, i.e. a `modlib` dependency this mod does not have. **The accepted cost is that 5.9/5.10 clients now show the wheel a half-turn out**, where before they were right; 5.11+ clients go from wrong to right. Revisit by giving `cars` the `modlib` treatment if pre-5.11 clients still matter. |
